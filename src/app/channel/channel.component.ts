...
import { typeUser, typeGroup, typeMessage } from '../models/types'
...
export class ChannelComponent implements OnInit {
  ...
  addUserToChannel() {
    if (this.channelName==='general') {alert('Cannot add users to default channel: general'); return}
    if (!this.newUsername) {alert('New user\'s username cannot be empty'); return}
    if (this.groupName==='newbies' || this.groupName==='general') {
      alert('Cannot add users in the default channels: newbies and general'); return
    }
  ...
  removeUser(usernameToRemove:string) {
    if (this.groupName==='newbies' || this.groupName==='general') {
      alert('Cannot remove users in this default channel'); return
    }
    if (usernameToRemove===this.username) {alert('Cannot remove yourself'); return}
    for (let user of this.allUsersData) {
      if (user.username===usernameToRemove && (user.groupAdmin || user.superAdmin)) {
        alert(`Cannot remove admin: ${usernameToRemove}`); return
      }
    }
