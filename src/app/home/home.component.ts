...
export class HomeComponent implements OnInit {
  ...
  getPanel() {
    this.socketPanelService.retrieveData().subscribe((data:any) => {
      if (data['success']) {
        ...
        //this.ticketsLeadersObj.forEach((ticket:typeProduct) => {
          //ticket.last_update = ticket.last_update.replace('T', ' ').split('.')[0] + ' GT'
        //})
      }
    })
  ...
  viewDetails(ticketName:string) {
    ...
    this.ticketsLeadersObj.forEach((ticket:typeProduct, index:number) => {
      if (ticket.pro_name===ticketName) this.ticketsLeadersObj[index].details = !this.ticketsLeadersObj[index].details
    })
  }
